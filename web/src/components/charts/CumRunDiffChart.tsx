...
function parseWeekData(cumRunDiffByWeek: Record<string, number> | undefined): { week: number; value: number }[] {
  if (!cumRunDiffByWeek) return [];

  return Object.entries(cumRunDiffByWeek)
    .map(([key, value]) => {
        ...
        {seriesData.map((series, index) => {
          ...
          return (
            ...
            >
              ...
              <circle cx={5} cy={itemHeight / 2 - 2} r={5} fill={series.color} />
              <text
                x={15}
                y={itemHeight / 2 + 1}
                style={{
...
export function CumRunDiffChart({
  homeTeamStats,
  awayTeamStats,
  homeAbbrev,
  awayAbbrev,
    ...
    return (
      <div className="border border-[var(--border)] rounded bg-[var(--card)] p-2 h-full flex items-center justify-center">
        <div className="text-center text-[var(--muted)]">
          <div className="text-sm font-bold mb-1">Cumulative Run Differential</div>
  ...
  const chartData = seriesColors.map(series => {
    ...
    const data =
      series.id === awayAbbrev
        ? awayData.map(d => ({ x: d.week, y: d.value }))
        : homeData.map(d => ({ x: d.week, y: d.value }));
  ...
  return (
    <div className="border border-[var(--border)] rounded bg-[var(--card)] p-2 h-full flex flex-col">
      <div className="text-center text-xs font-bold mb-1">Cumulative Run Diff by Week</div>
      <div className="flex-1 min-h-0">
        <ResponsiveLine
          ...
          yScale={{
            type: 'linear',
            min: leagueStats?.minCumRunDiff ?? 'auto',
            max: leagueStats?.maxCumRunDiff ?? 'auto',
            stacked: false,
          }}
          axisBottom={{
            tickSize: 0,
          ...
          tooltip={({ point }) => (
            <div className="bg-[var(--card)] border border-[var(--border)] px-2 py-1 rounded shadow-lg text-xs font-mono">
              <div className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full" style={{ backgroundColor: point.seriesColor }} />
