...
import { NHLTeamStats, LeagueCumXgStats } from '@/types/chart';
...
interface Props {
  ...
  leagueStats?: LeagueCumXgStats;
}

type CustomLayerProps = LineCustomSvgLayerProps<LineSeries>;

function parseWeekData(cumXgfPctByWeek: Record<string, number> | undefined): { week: number; value: number }[] {
  if (!cumXgfPctByWeek) return [];

  return Object.entries(cumXgfPctByWeek)
    .map(([key, value]) => {
...
export function CumXgChart({ homeTeamStats, awayTeamStats, homeAbbrev, awayAbbrev, tenthXgfPctByWeek, leagueStats }: Props) {
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const homeData = parseWeekData(homeTeamStats.cumXgfPctByWeek);
  const awayData = parseWeekData(awayTeamStats.cumXgfPctByWeek);
  const tenthData = parseWeekData(tenthXgfPctByWeek);
    ...
    return (
      <div className="border border-[var(--border)] rounded bg-[var(--card)] p-2 h-full flex items-center justify-center">
        <div className="text-center text-[var(--muted)]">
          <div className="text-sm font-bold mb-1">Cumulative xG%</div>
  ...
  const seriesColors = [
    ...
    ...(tenthData.length > 0 ? [{ id: '#8', color: '#22c55e' }] : []),
  ];
  ...
  return (
    <div className="border border-[var(--border)] rounded bg-[var(--card)] p-2 h-full flex flex-col">
      <div className="text-center text-xs font-bold mb-1">Cumulative xG% by Week</div>
      <div className="flex-1 min-h-0">
        <ResponsiveLine
          ...
          yScale={{
            type: 'linear',
            min: leagueStats?.minCumXgPct ?? 'auto',
            max: leagueStats?.maxCumXgPct ?? 'auto',
            stacked: false,
          }}
          axisBottom={{
            tickSize: 0,
          ...
          tooltip={({ point }) => (
            <div className="bg-[var(--card)] border border-[var(--border)] px-2 py-1 rounded shadow-lg text-xs font-mono">
              ...
              <div className="text-[var(--muted)]">
                Week {String(point.data.x)}: {Number(point.data.y ?? 0).toFixed(1)}%
              </div>
            </div>
          )}
        />
