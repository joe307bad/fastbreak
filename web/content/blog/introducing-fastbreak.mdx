...
I want an app that gives me the tools to contribute meaningfully to sports conversations. Too often, I form opinions from talking heads on TV or podcasts. These folks have access to advanced tools that are only available to the media. Fastbreak can be a gateway to deeper, more informed, stat driven conversations about sports amongst fans.
...
When I use a mobile app, I don't want it to be non-functional if I lose internet connection. Fastbreak will always work as long as you download the charts once when opening the app. Updates obviously need an internet connection, but the native iOS and Android apps are built to download all the latest data and cache it until the next update.
