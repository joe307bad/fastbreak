...
let calculateLeaderboard (database: IMongoDatabase) (statSheets: StatSheet list) (startDateCode: string): Task<LeaderboardResult> =
    task {
        ...
        let! dailyLeaderboards =
            ...
            |> List.map (fun dateCode ->
                task {
                    let! dailyScores =
                        ...
                        |> List.map (fun sheet ->
                            task {
                                ...
                                return { userId = sheet.userId; userName = userName; points = points }
                            }
                        )
                        |> Task.WhenAll
                    ...
                    return { dateCode = dateCode; entries = filteredAndSorted }
                }
            )
            |> Task.WhenAll
        ...
        let! weeklyTotals =
            ...
            |> List.map (fun sheet -> 
                task {
                    let! userName = getUserNameFromUserId database sheet.userId
                    return { userId = sheet.userId; userName = userName; points = sheet.items.currentWeek.total }
                }
            )
            |> Task.WhenAll
        ...
        return { dailyLeaderboards = dailyLeaderboards; weeklyTotals = filteredWeeklyTotals }
    }